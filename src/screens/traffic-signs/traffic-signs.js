...
import {
  ...
  Image,
  Platform,
  SafeAreaView, SectionList, StatusBar, TouchableOpacity,
  View
} from 'react-native';
...
const trafficList = [
  {
    ...
    code: "P.101",
    name: "Đường cấm",
    content: "Biển báo đường cấm tất cả các loại phương tiện tham gia giao thông đi lại cả hai hướng, trừ xe ưu tiên theo luật định."
  },
  {
    icon: DEFAULT_IMAGE,
    code: "P.102",
...
const TrafficSigns = () => {
  ...
  return (
    <SafeAreaView
      ...
      <NavigationBar title="Các biển báo" titleStyle={styles.titleStyle} statusBarStyle={styles.statusBarStyle} />
      <View>
        <SectionList
          sections={DATA}
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              onPress={() => onOpenTrafficSign(item.icon, item.code, item.name, item.content)}>
              <View style={styles.trafficSignIconBox}>
                <Image source={{uri: DEFAULT_IMAGE}} style={styles.trafficSignIcon} />
              </View>
